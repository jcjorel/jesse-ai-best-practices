...
class StrandsClaude4Driver:
    ...
    async def stream_conversation(self, message: str, conversation_id: str,
        ...
        if use_cache and self.config.enable_prompt_caching:
            ...
            if cached_response:
                ...
                async for chunk in self._simulate_streaming(cached_response):
                    yield chunk
                return
    ...
    async def _stream_with_retry(self, prompt: str) -> AsyncIterator[StreamingResponse]:
        ...
        for attempt in range(self.config.max_retries + 1):
            try:
                ...
                # For now, Strands doesn't expose streaming directly in the simple interface
                # So we'll use the regular call and simulate streaming
                result = await asyncio.get_event_loop().run_in_executor(
                    None, lambda: self._agent(prompt)
                )
    ...
    async def _simulate_streaming(self, response: str) -> AsyncIterator[StreamingResponse]:
        ...
        for i in range(0, len(response), chunk_size):
            ...
            yield StreamingResponse(
                ...
                metadata={"simulated": True, "chunk_index": i // chunk_size}
            )
