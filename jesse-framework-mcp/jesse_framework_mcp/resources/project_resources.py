...
async def get_project_gitignore_files(ctx: Context) -> str:
    """
    [Function intent]
    Project gitignore files resource returning mandatory and existing optional .gitignore contents as HTTP sections.
    
    [Design principles]
    Multi-file resource with criticality based on file importance.
    Mandatory files always included (with CRITICAL warning if missing).
    Optional files only included if they exist.
    
    [Implementation details]
    Scans mandatory locations (PROJECT_ROOT, .knowledge/git-clones) always,
    scans optional locations only if files exist, formats each as writable HTTP section.
    
    Args:
        ctx: FastMCP Context for progress reporting and logging
        
    Returns:
        Multi-part HTTP response with mandatory and existing optional .gitignore file contents
        
    Raises:
        ValueError: When gitignore files resource cannot be built
    """
